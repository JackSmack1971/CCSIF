# Git Automation Function Library
...
- [Repository health management](#repository-health-management)
- [Continuous learning integration](#continuous-learning-integration)
- [Error recovery patterns](#error-recovery-patterns)

Read-only functions (analysis, detection, health checks) may run without asking. Every function tagged **[MUTATING]** requires the human-confirmation gate in `../SKILL.md` before it runs. Every function tagged **[REMOTE/DESTRUCTIVE]** requires the explicit-approval gate — never invoke it automatically inside a single unattended turn.

## Repository intelligence

### Repository analysis (read-only)
```bash
analyze_repository() {
  local repo_path=$1
...
### Branching strategy detection (read-only)
```bash
...
### Semantic commit analysis (read-only)
```bash
...
# Suggest a commit message from staged changes (read-only; the user or caller decides whether to use it)
...
### Automated commit optimization **[MUTATING, history rewrite]**
```bash
# Rewrites local commit history. Requires human confirmation per SKILL.md.
optimize_commit_history() {
  ...
  # Removes empty commits by rewriting the last 50 commits. Never run on shared/pushed history without approval.
...
### Intelligent version bumping **[MUTATING]**
```bash
smart_version_bump() {
...
# Updates version strings across manifests. Confirm the target files with the user first.
...
### Release workflow automation **[REMOTE/DESTRUCTIVE]**

Pushes to `main`, pushes tags, and publishes a GitHub release. Run each numbered step only after the human-confirmation gate in `../SKILL.md` and only with the user's explicit go-ahead for the push and release steps specifically.
...
execute_release_workflow() {
  ...
  # 7. Push changes -- confirm explicitly before this line runs
  git push origin main
  git push origin "v$new_version"

  # 8. Create GitHub release -- confirm explicitly before this line runs
...
# Pre-release validation (read-only checks; safe to run automatically)
...
### GitHub integration **[REMOTE — create-pr and update-repo-info mutate the remote; merge-pr and create-release are DESTRUCTIVE/REMOTE]**
```bash
github_operations() {
  local operation=$1
  local repo_name=$2
  ...
  case $operation in
    ...
    "merge-pr")
      # Merges and deletes the source branch. Confirm explicitly before this runs.
      local pr_number=$2
...
### Repository cleanup **[MUTATING, irreversible object pruning]**
```bash
# Prunes unreachable objects and rewrites blob history above the size threshold.
# Confirm explicitly before running -- pruned objects and stripped blobs are not recoverable from this repository alone.
cleanup_repository() {
...
### Repository health analysis (read-only)
```bash
analyze_repository_health() {
  ...
  if [[ -n "$large_files" ]]; then
    echo "Found large files in repository:"
    echo "$large_files"
    ((issues++))
  fi
  ...
  if [[ $small_commits -gt 50 ]]; then
    echo "High number of small commits ($small_commits). Consider squashing."
    ((issues++))
  fi
...
Illustrative shape only; populate from real repository history, never from assumed defaults.
...
### Handle common Git operation failures (mostly read-only diagnosis; the `pull` branch mutates the working tree)
```bash
handle_git_failure() {
  local operation=$1
  local error_code=$2
