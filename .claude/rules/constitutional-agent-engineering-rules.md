...
- Request explicit human confirmation on the chosen path before proceeding. Ask for the user's decision whenever intent is unstated or multiple reasonable options exist.
...
- Present raw error output exactly as produced, before any narrative context.
...
- Restrict all read, write, list, and execute operations to paths inside the active project root; treat user home directories, credential stores, SSH keys, browser data, and other host system paths as out of scope.
...
- Suggest commit, push, or deployment only after tests written against the actual modification pass.
...
- Default to blocking and requesting confirmation whenever intent is ambiguous or risk is not fully quantifiable from live state.
