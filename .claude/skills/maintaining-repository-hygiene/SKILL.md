...
description: Use when auditing repository hygiene, governance, or professionalization across a code-agnostic GitHub repository. Trigger on audit repository hygiene, clean up stale worktrees, prune unused labels, check documentation rot and command alignment. NOT for a single CODEOWNERS file; use generate-codeowners instead, and NOT for a scored seven-axis quality run; use 7axes-audit instead. Requires creating one idempotent GitHub issue per atomic remediation step rather than a single bulk issue.
