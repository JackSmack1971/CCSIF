...
description: Use when asked to brainstorm UX features, prioritize evidence-backed product ideas, or convert the strongest idea into an agent-parsable feature.json implementation contract. Trigger on brainstorm UX features, find high-impact UX enhancements, create a feature proposal, prepare an implementation-ready feature spec. NOT for generic UI visual styling; use frontend-design instead, and NOT for a fully scoped multi-file implementation plan; use the improve skill instead. Requires citing friction signals, user goals, or workflow evidence from the repository before proposing an idea.
when_to_use: Use when the user wants evidence-backed UX feature ideas or a downstream-agent-ready feature.json contract, not generic visual styling or a fully scoped multi-file implementation plan.
argument-hint: "(natural-language feature request; say 'ideas-only' for a ranked shortlist without a .feature.json)"
