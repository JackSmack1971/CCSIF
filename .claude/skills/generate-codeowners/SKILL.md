...
description: Use when generating or auditing a GitHub CODEOWNERS file from verified repository evidence and GitHub ownership data. Trigger on queries that say generate CODEOWNERS, audit our CODEOWNERS file, redesign ownership mapping, fix a monorepo ownership routing problem, or resolve a review bottleneck. Analyzes repository architecture, history, risk boundaries, existing governance, and verified GitHub ownership data; it never invents GitHub handles or writes unresolved owners. NOT for GitLab approval sections or Bitbucket-specific ownership syntax use the platform's native tooling instead. Distinct keywords ownership plan schema, blank-owner rules, review bottleneck, verified owners, last-match precedence.
...
- [ ] Resolve the repository root and read the design policy and ownership-plan schema.
- [ ] Run the repository inventory and GitHub-owner discovery scripts.
- [ ] Resolve owners only from verified evidence; never invent handles.
- [ ] Write `ownership-plan.json`, then render CODEOWNERS atomically.
- [ ] Validate with `validate_codeowners.py` and fix until it exits zero.
- [ ] Confirm `git diff` is limited to `.github/CODEOWNERS` before reporting completion.

**Stop condition:** stop before writing `.github/CODEOWNERS` if any required production domain lacks a verified owner; report the ownership gap instead.
