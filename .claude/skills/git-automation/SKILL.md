---
name: git-automation
description: Use when automating Git repository management beyond a single commit, including intelligent branching-strategy detection, commit-message and history optimization, release workflows, or repository health cleanup. Trigger on queries that say automate our release workflow, detect our branching strategy, clean up repository health, or bump the version across manifests. NOT for a single conventional commit use the git-commit skill instead, and NOT for opening or reviewing one pull request use create-pr or review-pr instead. Requires explicit human confirmation before any branching, tagging, or history-rewriting operation runs.
