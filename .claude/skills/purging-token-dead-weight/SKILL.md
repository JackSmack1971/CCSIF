...
**Stop condition:** stop before any deletion if the mode is `apply` without the exact confirmation token `PURGE`, or if the resolved root is not a Git repository.
