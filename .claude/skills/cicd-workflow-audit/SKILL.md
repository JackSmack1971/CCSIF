...
description: Use when auditing CI/CD pipeline definitions, workflow trigger safety, deployment gates, token permissions, secret handling, cache/artifact policy, third-party action pinning, or missing test/lint/typecheck coverage. Trigger on queries that say audit CI, review GitHub Actions, inspect GitLab CI, review CircleCI config, check deployment workflow safety, or compare pipeline jobs to repository manifests. Produces actionable findings with file paths, workflow/job names, evidence, impact, and remediation. NOT for directly modifying protected CI/CD deployment workflows unless explicitly approved. Distinct keywords trigger safety, pull_request_target, deployment gates, workflow permissions, action pinning, artifact retention.
