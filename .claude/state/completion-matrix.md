...
| Criterion | Status | Evidence | Notes |
|---|---|---|---|
| The repo has a clear project `CLAUDE.md` and `.claude/rules/` layout; nothing depends on `~/.claude/`. | existing | `CLAUDE.md`; `.claude/rules/*.md`; `.claude/settings.json`; `docs/claude-code-control-plane-roadmap-v2.md` | Repo scan found no active `~/.claude/*` dependency. |
| Repeated procedures are split into project skills instead of duplicated in chat or root docs. | partial | `.claude/skills/*`; `.claude/README.md` | Many skills exist, but the baseline does not prove deduplication discipline or promotion rules. |
| Required external tools are available through approved MCP servers declared in `.mcp.json`. | existing | `.mcp.json`; `.claude/settings.json`; `.claude/memory/hindsight_mcp.py` | A project-scoped MCP server is declared and wired. |
| Unsafe or out-of-policy tool calls are blocked or annotated by project-scoped hooks before the next loop step. | existing | `.claude/hooks/pre-tool-use.sh`; `.claude/hooks/lib/pre-tool-use-guard.js`; `.claude/hooks/post-tool-use.sh`; `.claude/scripts/control_plane_check.py` | The baseline check exercises protected and allowed probes successfully. |
| Delegated subagent work uses the Agent tool pattern with role definitions in `.claude/agents/`. | partial | `.claude/agents/implementation-agent.md`; `.claude/agents/pr-reviewer.md`; `.claude/agents/reflect-agent.md`; `.claude/agents/upstream-auditor.md` | Role files exist, but there is no runtime proof of Agent-tool delegation yet. |
